...
//mport { filterH } from '../contact/contact.component';
...
btnExportInstance: any;
 ...
 ;
  ...
  openModal(e){
    ...
        // this.importModule.display="block";
...
refreshDataGrid() {
 ...
 /*   this.ContactClient = new CustomStore({
      key: "id",
      load: function (loadOptions: any) {
       ...
       loadOptions.select= "id";
          ...
          if (q.length != 0) f += '(((is_active:true) AND (contactType:Client)) AND (' + q[0];
          for (let i = 1; i < q.length; i++) {
              f += " AND " + q[i];
          }
          if (q.length != 0) f += '))';
          if ((f != "") && reqRechercher.length != 0) f += "AND"
          if (reqRechercher.length != 0) f += '(((is_active:true) AND (contactType:Client)) AND (' + reqRechercher[0];
          ...
          if (loadOptions.filter) {
              ...
              loadOptions.searchValue = "(* AND (is_active:true) AND (contactType:Client))";
          }
...
*/
  ...
  getListHeaderFilter (type,list): any
  {
   ...
   for (let t of data) {
               if(t[type]!="" && t["contactType"]=="Client" && t["isActive"] && t[type]!=null){
                f.push(t[type]);
               }
