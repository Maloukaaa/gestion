...
export class ContactPartenaireComponent implements OnInit {
  contacts: Contact[];
  public ContactPartenaire;
...
public goShow(e,id)
{
  ...
  constructor(private data: DataService,protected contactService: ContactService, private translate: TranslateService,private router:Router,private http:HttpClient) {
    ...
    this.ContactPartenaire = new CustomStore({
      key: "id",
      load: function (loadOptions: any) {
       ...
       loadOptions.select= "id";
          ...
          if (q.length != 0) f += '(((is_active:true) AND (contactType:Partenaire)) AND (' + q[0];
          for (let i = 1; i < q.length; i++) {
              f += " AND " + q[i];
          }
          if (q.length != 0) f += '))';
          if ((f != "") && reqRechercher.length != 0) f += "AND"
          if (reqRechercher.length != 0) f += '(((is_active:true) AND (contactType:Partenaire)) AND (' + reqRechercher[0];
          ...
          if (loadOptions.filter) {
              ...
              loadOptions.searchValue = "(* AND (is_active:true) AND (contactType:Partenaire))";
          }
  ...
  this.ListeO=[];
   ...
   getListHeaderFilter (type,list): any
   {
    ...
    for (let t of data) {
                if(t[type]!="" && t["contactType"]=="Partenaire"  && t["isActive"] && t[type]!=null){
                 f.push(t[type]);
                }
